...
{
    $Admin = Admin::whereNotNull('schname')->get();
...
public function import(Request $request)
{
    ...
    foreach ($rows as $row) {
        ...
        // Skip rows that have all values as null
        ...
        if (!empty($missingFields)) {
            $errors[] = "Missing values for fields: " . implode(', ', $missingFields);
        ...
        $validator = Validator::make($data, [
            ...
            'onames' => 'string',
            'addy' => 'string',
            'dob(yyyy-mm-dd)' => 'date_format:Y-m-d', // Keep as is, but it's not required
            'parentname' => 'string',
            'parentno' => 'string',
            'parentemail' => 'email',
        ]);
        ...
        if ($validator->fails()) {
            ...
            $errors[] = $validator->errors()->all(); // Store all errors for this row
            continue; // Skip to the next row
        }
    ...
    // If there are any errors, return with errors
    if (!empty($errors)) {
...
public function importpaye(Request $request)
{
    ...
    if (empty($header) || !in_array('fullname', $header) || !in_array('amt', $header)) {
        return redirect()->back()->with('error', 'Invalid CSV format. Please check your file.');
    }
...
public function viewpay(Request $request)
{
  ...
  // Fetch all sessions for lookup
  $seszions = seszion::all()->keyBy('id'); // Key by ID for easy lookup
    ...
    public function importexam(Request $request)
{
    ...
    $successMessage = $importedRecords > 0 || $updatedRecords > 0
        ? "exam scores were successfully imported and updated where specified for the subject: $subjectName."
